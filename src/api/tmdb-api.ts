...
export const getMovies = (page = 1, language = "en-US") => {
  return fetch(
    ...
    }&language=${language}&include_adult=false&include_video=false&page=${page}` // API endpoint for discovering movies
  )
    .then((response) => {
      if (!response.ok)
        throw new Error(
...
export const getUpcomingMovies = (page = 1, language = "en-US") => {
  return fetch(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${
      import.meta.env.VITE_TMDB_KEY
    }&language=${language}&page=${page}` // API endpoint for upcoming movies
  )
    .then((res) => res.json())
...
export const getPopularActors = (page = 1, language = "en-US") => {
  return fetch(
    `https://api.themoviedb.org/3/person/popular?api_key=${
      // Fetching popular actors
      import.meta.env.VITE_TMDB_KEY
    }&language=${language}&page=${page}` // API endpoint for popular actors
  )
    .then((response) => {
      if (!response.ok)
        throw new Error(
